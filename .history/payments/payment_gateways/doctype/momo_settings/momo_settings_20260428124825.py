...
def finalize_pr_payment(payment_request_name, reference_id):
    """
    Finalize an MTN-confirmed Payment Request:
      1. Mark Integration Request as Completed.
      2. Create + submit a Payment Entry against the PR's reference doc.
         (set_as_paid() short-circuits PE creation when payment_channel='Phone',
          which is what MoMo registers, so we call create_payment_entry directly.)
      3. Mark the PR as Paid.
      4. Fire on_payment_authorized on the reference doc for any user-defined hook.
    Idempotent: returns early if PR is already Paid.
    """
    ...
    try:
        ...
        if pr.status == "Paid":
            return {"payment_request": pr.name, "redirect_to": _pr_success_redirect(pr)}
        ...
        pr.db_set({"status": "Paid", "outstanding_amount": 0}, update_modified=True)

        if pr.reference_doctype and pr.reference_name and frappe.db.exists(pr.reference_doctype, pr.reference_name):
            ref = frappe.get_doc(pr.reference_doctype, pr.reference_name)
        ...
        return {
            ...
            "redirect_to": _pr_success_redirect(pr),
        }
    ...
    if data.get("status") == "SUCCESSFUL":
        ...
        try:
            ...
            if integration_request.reference_doctype == "Payment Request":
                # Reuse the same finalizer the polling path uses; idempotent if already Paid.
                finalize_pr_payment(integration_request.reference_docname, ref_id)
